...
const universalChangeHandler = (event: React.ChangeEvent<any>, state: any, setState: (v:any) => void) => {
...
export const Field = ({name, component: Component = 'input', type, value: valueProp, children, ...rest}: FieldProps) => {
    ...
    if (type === 'checkbox') {
        if (Array.isArray(value)) {
            return (
              <Component {...rest} name={name} type={type} value={valueProp} checked={value.includes(valueProp)} onChange={changeHandler} onBlur={blurHandler}>
                {children}
              </Component>);
        } else {
            return (<Component {...rest} name={name} type={type} checked={value} onChange={changeHandler} onBlur={blurHandler}>
                {children}
            </Component>);
        }
    } else if (type === 'radio') {
        return (<Component {...rest} name={name} type={type} checked={value === valueProp} value={valueProp} onChange={changeHandler} onBlur={blurHandler}>
            {children}
        </Component>);
    } else {
        return (<Component {...rest} name={name} type={type} value={value} onChange={changeHandler} onBlur={blurHandler}>
            {children}
        </Component>);
    }
};
...
export const useField = ({name, type, value: valueProp, ...rest}: any) => {
    ...
    const fieldProps: any = {...rest, name, type, onChange: changeHandler, onBlur: blurHandler};
